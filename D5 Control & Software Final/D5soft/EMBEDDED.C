...
int main ()
{
	...
	current=(int*)malloc(bit*sizeof(int));
	   ...
	   //power and energy*****************************************************************************************************************
    int *power;
    uint16_t power_new,power_old=0;
	...
	display_description(10,75,power1);
	display_description(130,75,energy1);
	...
	*battery_value=0;
	...
	for(;;){
		...
		for(i=0;i<=9;i++){
			...
			if(resultvoltage>0){
			*voltage=(resultvoltage*3.3/1024)*1.55*1000;
		...
		*power=(*current)*(*voltage);
		...
		display_value(60,75,powerstring,Watt); //IBusBar
		...
		if(busvoltage<=0.05){
			*main_value=0;
			...
			j++;
				...
				display_value(180,75,totalpowerstring,cWh);//energy display with battery
